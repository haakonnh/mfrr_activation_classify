...
information about the person in terms of a target variable, e.g., income.
In this project, each data point represents a specific timestamp in the mFRR
...
intervals old at the target delivery time.
...
However, since mFRR imbalance volumes are known to be highly autocorrelated
...
An additional complication is that some data sources are made available only at
...
This data is thus not only available for the target time $t+4$. This complicates
...
availabilities have changed slightly. Balance market activation data 
...
Down-activations. To capture this behavior succinctly, a set of persistence
...
Figure \ref{fig:persistence_illustration}. The same applies for up-activations and no activations. These
...
Persistence features capture consecutive activation trends effectively, but they
...
that the models developed in this project are able to capture these kinds of
...
the model understand how mFRR activations vary with time, such as daily or
...
as the day-ahead market, the intraday market, and the aFRR market. While most prices
may not directly impact activations, by constructing features that
capture relationships between prices, the model may infer
system stress levels that could lead to mFRR activations. Large discrepancies
...
\ref{fig:realized_wind_mfrr} and \ref{fig:wind_share_mfrr} show values of realized wind production and wind share (wind
...
distribution.}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{Wind share plotted against mFRR activation
distribution.}
