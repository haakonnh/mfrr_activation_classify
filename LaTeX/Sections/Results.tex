...
\autocite{statnettConfirmationMFRREAM2025}. The shorter dataset avoids these issues but
...
\subsubsection{Classification Threshold Adjustment Impact}\label{sec:classification-threshold-adjustment-results}
The classification threshold adjustment methodology described in Section
\ref{sec:classification-threshold-adjustment} made among the most significant
improvements to model performance during development. With default thresholds,
most models avoided predicting up-activations to a major extent. By up-scaling
up-activation probabilities post-training, the model is encouraged to put more
weight on up-activation indicators. The thresholds were tuned on the validation
set, configured to optimize F1-macro score. Figure
\ref{fig:catboost-confusion-val-test} and Table \ref{tab:catboost-val-test} show
results for the CatBoost model with adjusted thresholds. Pre-adjustment models
consistently showed very low recall for up-activations (often $<$0.2), lowering
F1-macro scores significantly. Threshold adjustments, though tuned on a
particular validation set, generalized well to the test set, improving
up-activation recall notably without sacrificing too much precision.
...
\bottomrule
\end{tabular}
\caption{Summary statistics for delivery-time spreads (CatBoost), restricted to correctly predicted UP/DOWN cases on test set.}
...
\bottomrule
\end{tabular}
\caption{Summary statistics for delivery-time spreads (Naive Model), restricted to correctly predicted UP/DOWN cases on test set.}
\label{tab:price-spread-stats-tail20}
\end{table}

\paragraph{Comparison with Naive Model.}\label{sec:price-diff-naive-results}
 To assess whether the machine learning
model captures more profitable activation opportunities than the naive model,
the same price difference analysis was performed for the naive model's correct
predictions. Table \ref{tab:price-spread-stats-tail20}, when compared with Table
\ref{tab:price-spread-stats} on the test set, shows that the machine learning
model captures more profitable up-activations (mean 17.98 EUR/MWh vs 15.83
EUR/MWh) and down-activations (mean -12.34 EUR/MWh vs -11.34 EUR/MWh) than the
naive model. This indicates that the machine learning model is able to identify
more lucrative activation opportunities beyond what is captured by simple
...
The entire codebase for data processing, feature engineering, model training, and
evaluation is available in a public GitHub repository: 
\url{https://github.com/haakonnh/mfrr_activation_classify}. The repository is 
segmented into various modules, each corresponding to different stages of the
project workflow. Comprehensive documentation is provided within the repository,
including a README file that outlines the project structure, dependencies, and
instructions for reproducing the results presented in this report. 
