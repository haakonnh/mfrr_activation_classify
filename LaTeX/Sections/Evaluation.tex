...
is one more evaluation metric which is important to consider for this specific
problem: how often the model manages to predict \textit{transitions} between 
classes. This is measured by looking at all sequential time interval pairs 
$t-4 and t+4$ with differing class labels. If the model predicted the class
label for time interval $t+4$ correctly, it is counted as a successfully
predicted transition. This is a crucial metric as it indicates its non-persistence
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/data_splitting.png}
...
dataset and the problem at hand. \textbf{Section not finished.}
...
The rationale behind this methos is that the model picks up on up-activation
...
minority class based on validation set performance. \textbf{Discuss somehwere in
the report that perhaps some internal cross-validation-based tuning could be
better.}
