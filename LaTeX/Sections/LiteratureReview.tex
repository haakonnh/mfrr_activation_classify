...
This chapter reviews literature relevant to balancing-market forecasting. It
...
events. Participants must also bid in the correct direction of activation
(upward or downward) to be eligible for activation. When an up-regulating
...
activated energy volumes. This target is crucial, directly informing bidding
...
imbalance-volume forecasting studies that before assessing how each
...
imbalance-volume forecasting from a market-participant-oriented standpoint.
Additionally, they develop a LSTM model, including several Nordic bidding zones.
Their analysis yields three relevant insights. First, balancing volumes are
relatively auto-correlated: past imbalances contain predictive information about
short-term imbalances. Second, they note that forecast accuracy could likely be
improved by incorporating weather-related variables. Third, they stress that
zero-regulation dominates the dataset, meaning the model must infer relatively
infrequent activation events from a mostly inactive baseline. This is an
...
This study therefore takes the perspective that direction modelling is not only
useful by itself (because bids must be directional), but also a stepping stone
to better volume and price forecasts: a two-stage direction-volume approach
first resolves the discrete directional uncertainty, then estimates a
conditional imbalance volume. The subsequent sections review how existing literature
has addressed activation direction uncertainty.
...
\textbf{This section confuses me, should it even be in this section?
Does not model activation uncertainty in my eyes, but rather uses imbalance
forecasts as a prerequisite (which already must account for directional
uncertainty.)}
...
The usage of chance constraints is a mathematical optimization technique used to
...
Pav\'ic et al. (2023) argue that deterministic reserve activation models
...
\subsubsection{Activation Direction Classification Models}
...
direction. Conclusively, they suggest that closer-to-real-time predictions could
...
\\
\midrule
Edling \& Azarang (2025) & mFRR activation volume & 1-hour & ML point forecast
(LSTM) \\
\midrule
Backe et al.\ (KoBas) (2023) & Imbalance volume & 1-hour & Probabilistic LSTM;
implicit direction \\
\midrule
Plakas et al.\ (2025) & Imbalance volume and price & 1-hour & Probabilistic
(quantile regression) \\
\midrule
Bankefors (2024) & Signed activation volume & 1-hour & Linear ML time-series;
implicit direction \\
%\midrule
...
\midrule
Irrmann (2023) & Direction and volumes & 1-hour & Expected activation ratios;
sampled regulation states \\
\midrule
Papavasiliou et al.\ (2022) & Reserve sufficiency & Scenario-based & Chance
constraints on imbalance scenarios \\
\midrule
Browell (2018) & System length (direction) & 1-hour & Probabilistic logistic
model with risk constraints \\
\midrule
Pavić et al.\ (2023) & Reserve activation (RA ratio) & 30-min (FCR/aFRR) &
Activation ranges / bounded uncertainty sets \\
\midrule
Klæboe et al.\ (2015) & Balancing state (up/down/none) & 1-hour & Markov
transition probabilities (duration/hour-specific) \\
\midrule
Svedlindh \& Yngveson (2025) & Activation direction (day-ahead) & 1-hour &
Classification (logistic, ANN) \\
\midrule
Porras (2025) & Activation direction and price & 1-hour & Classification
...
Despite notable progress and coverage of balancing-market forecasting,
a couple of gaps emerge from the literature. The simplest observation is that
most studies were conducted before the Nordic system's transition to 15-minute 
mFRR MTU in March 2025. As discussed, this change alters the market dynamics 
and activation patterns. While hourly forecasts remain relevant, it remains 
unclear how well existing models perform at the higher resolution.

The literature focuses predominantly on continuous imbalance volumes or prices 
as target variables. These quantities are of great importance, but they 
are conditional on activation direction, which is not modelled explicitly in
most studies. Results are therefore affected by directional uncertainty, making
...
This study argues that explicit direction modelling is useful for market
participants by itself, but also as a stepping stone towards improved volume and
price forecasting through a two-stage direction-then-volume modelling approach.
