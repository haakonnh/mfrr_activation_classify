...
def resolve_repo_root() -> Path:
	"""Resolve repo root assuming this file is at mfrr_classify/scripts/.

	Falls back to CWD if the expected layout is not present.
	"""
	here = Path(__file__).resolve()
	# mfrr_classify/scripts/wipe_logs.py -> repo root is parents[2]
	cand = here.parents[2]
	if (cand / 'mfrr_classify').exists():
		return cand
	return Path.cwd()
...
def main():
	...
	logs_dir = repo / 'mfrr_classify' / 'models' / 'time_curve_logs'
	runs_dir = repo / 'mfrr_classify' / 'models' / 'time_curve_runs'
